...
ct_path = "/content/Screenshot 2025-10-22 160549.png" mri_path = "/content/Screenshot 2025-10-22 160636.png" 
ct = cv2.imread(ct_path, cv2.IMREAD_GRAYSCALE) mri = cv2.imread(mri_path, cv2.IMREAD_GRAYSCALE) 

print(f"Error: Could not load CT image from {ct_path}") elif mri is None: 
print(f"Error: Could not load MRI image from {mri_path}") else: 
...
plt.title(f"Fused Image\nEstimated Diagnostic Accuracy: {diagnostic_accuracy:.2f}%") 
plt.tight_layout() 
plt.show() 
